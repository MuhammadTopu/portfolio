...
const STATIC_PROJECTS = [
  {
    ...
    img: "/old/assets/images/project-1.jpg",
  },
];
...
function ProjectCard({ project }) {
  ...
  return (
    <Tag
      {...linkProps}
      className={`block group ${project.link ? "cursor-pointer" : "cursor-default"}`}
    >
      <figure className="relative sketch-border overflow-hidden mb-4 aspect-4/3 bg-card p-1 group-hover:translate-x-1 group-hover:translate-y-1 transition-all duration-300 group-hover:bg-primary-light">
        ...
        {project.img ? (
          ...
          <div className="w-full h-full bg-primary-light flex items-center justify-center text-muted text-xs font-bold uppercase tracking-widest">
            {project.category}
          </div>
        )}
      </figure>
      <div className="flex items-start justify-between gap-2 ml-1">
        <div>
          <h3 className="text-xl font-signature font-bold text-foreground mb-1">
            {project.title}
          </h3>
          <p className="text-xs lg:text-sm text-muted font-bold uppercase tracking-wider">
            {project.category}
          </p>
          {project.desc && (
