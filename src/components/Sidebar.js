...
export function Sidebar() {
  ...
  return (
    <>
      <aside
        className={`bg-sidebar border-2 border-card-border sketch-border p-4 lg:p-6 transition-[max-height] duration-500 ease-in-out relative z-10 lg:sticky lg:top-[60px] lg:mb-0 lg:w-[280px] lg:shrink-0 lg:flex lg:flex-col lg:justify-center lg:max-h-[calc(100vh-120px)] lg:overflow-y-auto custom-scrollbar overflow-hidden ${isActive ? "max-h-[800px]" : "max-h-[110px]"}`}
      >
        {/* Top info */}
        <div className="sidebar-info flex items-center gap-[20px] relative lg:block lg:text-center">
          ...
          <div className="info-content lg:mt-5 text-left lg:text-center">
            <h1
              className="name text-2xl lg:text-4xl font-signature font-bold tracking-tight text-foreground mb-1"
              title={data.name}
            >
              {data.name}
            </h1>
            <p className="title bg-foreground text-background text-[9px] lg:text-[10px] font-bold px-3 py-1  w-fit lg:mx-auto tracking-wide uppercase">
              {data.title}
            </p>
          </div>
        ...
        {/* Expandable section */}
        <div
          className={`sidebar-info_more ${isActive ? "block" : "hidden"} lg:block transition-all duration-500`}
        >
          ...
          {resumeModal && (
            ...
            >
              ...
              >
                ...
                <h3 className="font-signature font-bold text-2xl text-foreground">
                  Resume ain&apos;t available here
                </h3>
                <p className="text-muted text-sm leading-relaxed">
                  You gotta ask from me — reach out via email or LinkedIn and
                  I&apos;ll send it over.
                </p>
                <button
                  onClick={() => setResumeModal(false)}
          ...
          <ul className="contacts-list space-y-[20px]">
            {contacts.map((contact, idx) => (
              <li
                key={idx}
                className="contact-item flex items-center gap-3 group cursor-pointer"
              >
                ...
                <div className="contact-info text-left overflow-hidden min-w-0">
                  <p className="contact-title text-[10px] text-muted uppercase tracking-widest mb-0.5">
                    {contact.title}
                  </p>
                  {contact.link ? (
                    <a
                      href={contact.link}
                      className="contact-link text-[11px] text-foreground hover:text-primary transition-colors font-medium block truncate"
                    >
                      {contact.value}
                    </a>
                  ) : (
          ...
          <div className="mt-5 flex sketch-border overflow-hidden">
            {/* Left — use template */}
            <a
              href="https://github.com/MuhammadTopu"
              target="_blank"
              rel="noopener noreferrer"
            ...
            >
              ...
              <span className="flex flex-col">
                <span className="text-[9px] font-bold tracking-widest uppercase opacity-60 leading-none mb-0.5">
                  open source
                </span>
                <span className="font-signature font-bold text-sm leading-tight group-hover:underline underline-offset-2">
                  Use this template
                </span>
              </span>
            </a>
            ...
            {/* Right — star */}
            <a
              href="https://github.com/MuhammadTopu/portfolio"
              target="_blank"
              rel="noopener noreferrer"
            ...
            >
              <svg
                ...
                fill="currentColor"
                viewBox="0 0 24 24"
                className="group-hover/star:scale-125 group-hover/star:fill-yellow-400 transition-all duration-300"
              >
                <path d="M12 .587l3.668 7.568 8.332 1.151-6.064 5.828 1.48 8.279-7.416-3.967-7.417 3.967 1.481-8.279-6.064-5.828 8.332-1.151z" />
              </svg>
              <span className="text-[9px] font-bold tracking-widest uppercase opacity-70 leading-none">
                Star
              </span>
            </a>
          </div>
